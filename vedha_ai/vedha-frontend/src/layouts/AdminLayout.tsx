    ...
    <div className="flex h-screen w-screen overflow-hidden bg-slate-950 text-white font-sans">
      ...
      >
        ...
        <div className="flex h-20 items-center justify-between border-b border-slate-800 p-6">
          <div>
            <h1 className="text-2xl font-black bg-gradient-to-r from-violet-500 to-fuchsia-500 bg-clip-text text-transparent">
              Vedha AI
            </h1>
        ...
        <div className="border-b border-slate-800 p-6">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-600 font-bold text-white shadow-md shadow-violet-500/20">
              ...
              <NavLink
                ...
                className={() =>
                  `flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-all duration-200 ${
                    isActive
                      ? "bg-gradient-to-r from-violet-500/20 to-fuchsia-500/10 border border-violet-500/30 text-violet-300 shadow-inner"
      ...
      <div className="flex min-w-0 flex-1 flex-col">
              ...
              {pathnames.slice(1).map((value, index) => {
                ...
                return (
                  <div key={to} className="flex items-center gap-2">
                    ...
                    {isLast ? (
                      <span className="font-medium text-violet-400">{formatted}</span>
                    ) : (
          ...
          <div className="flex items-center gap-4">
            <div className="relative hidden max-w-xs md:block">
              ...
              <input
                ...
                className="w-64 rounded-xl border border-slate-800 bg-slate-900/60 pl-9 pr-4 py-2 text-xs text-white placeholder-slate-500 outline-none transition focus:border-violet-500"
              />
            </div>
            ...
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-600 font-bold text-white">
              {user?.name?.charAt(0).toUpperCase() ?? "A"}
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-8">
          ...
          <footer className="mt-16 border-t border-slate-900 py-6 text-center text-xs text-slate-500">
            Vedha AI © {new Date().getFullYear()} • Platform Operations Console
          </footer>
        </main>
      </div>
    </div>
